...
public record GameTeamConfig(
    ...
    public static final MapCodec<GameTeamConfig> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    public ItemStack createFirework(int flight, FireworkItem.Type type) {
        var color = this.fireworkColor().rgb;
        return ItemUtil.createFirework(color, flight, type);
    }

    public ItemStack applyDye(ItemStack stack) {
        return ItemStackBuilder.of(stack)
                .setColor(this.dyeColor().rgb)
                .build();
    }
