...
/**
 * Simple, {@link GameLogic} specific team manager class.
 */
...
public final class TeamManager implements Iterable<GameTeam> {
    ...
    /**
     * Creates and applies a {@link TeamManager} instance to the given {@link GameLogic}.
     *
     * @param activity the activity to apply teams to
     * @return the constructed {@link TeamManager}
     */
    public static TeamManager addTo(GameLogic activity) {
        var manager = new TeamManager(activity.getGameSpace());
        activity.listen(PlayerAddListener.EVENT, manager::onAddPlayer);
        activity.listen(PlayerRemoveListener.EVENT, manager::onRemovePlayer);
        activity.listen(PlayerDamageListener.EVENT, manager::onDamagePlayer);
    ...
    private void sendTeamsToPlayer(ServerPlayerEntity player) {
        for (var state : this.teamToState.values()) {
            player.getScoreboard().updateScoreboardTeam(state.scoreboardTeam);
    ...
    private void sendRemoveTeamsForPlayer(ServerPlayerEntity player) {
        for (var state : this.teamToState.entrySet()) {
            ...
            player.getScoreboard().updateRemovedTeam(data.scoreboardTeam);
    ...
    private void addOnlinePlayer(ServerPlayerEntity player, State state) {
        ...
        player.getScoreboard().addPlayerToTeam(player.getEntityName(), state.scoreboardTeam);
    ...
    private void removeOnlinePlayer(ServerPlayerEntity player, State state) {
        ...
        player.getScoreboard().removePlayerFromTeam(player.getEntityName(), state.scoreboardTeam);
        this.sendPacketToAll(this.resetPlayerName(player));
    }
