...
public final class TeamSelectionLobby {
    ...
    /**
     * Applies this team selection lobby implementation to the given {@link GameSpace}.
     *
     * @param logic the game to apply this lobby to
     * @param teams the teams to be available for selection
     * @return a new {@link TeamSelectionLobby} which can have teams added to it
     * @see TeamSelectionLobby#allocate(BiConsumer)
     */
    public static TeamSelectionLobby applyTo(GameLogic logic, GameTeamList teams) {
        TeamSelectionLobby lobby = new TeamSelectionLobby(logic.getGameSpace(), teams);

        logic.listen(PlayerAddListener.EVENT, lobby::onAddPlayer);
        logic.listen(UseItemListener.EVENT, lobby::onUseItem);
    ...
    public void allocate(BiConsumer<GameTeamKey, ServerPlayerEntity> apply) {
