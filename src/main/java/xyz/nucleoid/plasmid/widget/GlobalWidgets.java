 ...
 *
 * @see GlobalWidgets#addTo(GameLogic)
 * @see SidebarWidget
 * @see BossBarWidget
 */
public final class GlobalWidgets implements AutoCloseable {
    ...
    public GlobalWidgets(GameSpace gameSpace) {
        this.gameSpace = gameSpace;
    }

    /**
     * Creates a {@link GlobalWidgets} instance and registers it to the given {@link GameLogic}.
     * All players within this activity will have the added widgets displayed to them.
     *
     * @param logic the game to add this instance to
     * @return a {@link GlobalWidgets} instance which can be used to add various widgets for all players
     */
    public static GlobalWidgets addTo(GameLogic logic) {
        var widgets = new GlobalWidgets(logic.getGameSpace());

        logic.listen(PlayerAddListener.EVENT, widgets::onAddPlayer);
        logic.listen(PlayerRemoveListener.EVENT, widgets::onRemovePlayer);
        logic.listen(GameCloseListener.EVENT, widgets::close);
